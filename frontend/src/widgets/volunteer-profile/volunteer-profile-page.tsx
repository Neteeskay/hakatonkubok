          ...
          <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
            {profileBadges.map((badge) => (
              <div key={badge.title} className="text-center">
                <div className="mx-auto grid size-20 place-items-center rounded-[1.4rem] bg-[#171717] text-brand shadow-[0_16px_34px_rgba(34,28,8,0.16)]">
                  <badge.icon className="size-9" />
                </div>
                <p className="mt-3 text-sm font-black">{badge.title}</p>
                <p className="text-xs text-black/50">{badge.text}</p>
              </div>
            ))}
          </div>
          <div className="mt-6 rounded-[1.2rem] bg-[#fbfaf4] p-4">
            <div className="flex items-center gap-3">
              ...
              <div className="min-w-0 flex-1">
                <p className="text-sm font-black">До следующего бейджа «Доброе сердце»</p>
                <p className="mt-1 text-xs text-black/50">Ещё 19 часов</p>
                <div className="mt-3 h-2 rounded-full bg-[#e8e4f8]"><div className="h-full w-[74%] rounded-full bg-[#6b4de6]" /></div>
              </div>
              <p className="text-sm font-black text-black/58">56 / 75</p>
            </div>
          </div>
        </ProfileCard>

        <ProfileCard>
          <ProfileSectionTitle title="Последние достижения" action="Все достижения →" actionHref="/volunteer/achievements" />
          <div className="space-y-4">
            {profileAchievements.map((achievement) => (
              <div key={achievement.title} className="flex items-center gap-4">
                <RoundIcon icon={achievement.icon} tone={achievement.tone as "gold" | "green" | "violet"} />
                <div className="min-w-0 flex-1">
                  <p className="font-black">{achievement.title}</p>
                  <p className="mt-1 text-sm text-black/52">{achievement.text}</p>
                </div>
                <span className="text-sm font-black text-[#247a31]">{achievement.value}</span>
              </div>
            ))}
          </div>
        </ProfileCard>
