...
import { ArrowRight, Building2, ListChecks, Search } from "lucide-react";
import { adminFoundations, adminTasks, foundationStatusConfig, taskStatusConfig } from "@/widgets/admin/admin-data";
...
export function AdminTasksPage() {
  const [query, setQuery] = useState("");
  const visible = useMemo(() => adminTasks.filter((item) => `${item.title} ${item.foundation} ${item.category}`.toLowerCase().includes(query.toLowerCase())), [query]);
    ...
    <AdminPageShell eyebrow="Задания" title="Все активности платформы" description="Опубликованные, завершённые, возвращённые и ожидающие проверки задания в одном рабочем списке.">
      ...
      <section className="grid gap-4 xl:grid-cols-2">
        {visible.map((task) => {
          ...
          return (
            <article key={task.id} className="rounded-[1.6rem] bg-white p-5 shadow-[0_20px_60px_rgba(34,28,8,0.05),inset_0_0_0_1px_rgba(24,20,7,0.055)]">
              ...
              <button className="mt-5 inline-flex h-11 items-center justify-center gap-2 rounded-xl bg-[#fffdf7] px-4 text-sm font-black text-black shadow-[inset_0_0_0_1px_rgba(24,20,7,0.07)]">
                Открыть детали
